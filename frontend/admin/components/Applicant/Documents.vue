                            ...
                            <template v-slot:default>
                                ...
                                <template v-if="documents.length>0">
                                    <tr v-for="doc in documents">
                                     ...
                                     <td><v-chip small :class="doc.verified=='PENDING' ? 'warning' :'success'">{{ doc.verified }}</v-chip></td>
  ...
  export default{
    ...
    methods:{
        async viewfiles(filename){
         ...
         open(this.$store.state.servers.documentlink+'/'+filename,'test',params)
         
                
        },
        async verify(doc){
            await this.$store.dispatch("applicantdocuments/verify",doc); 
        }
    }

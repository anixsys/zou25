    ...
    <div>
      ...
      <v-dialog v-model="AddModel" width="500px">
        ...
        <v-card>
          ...
          <v-card-text>
            <v-select outlined label="Select Type" v-model="form.Type" :items="['NORAMAL REG','LATE REG']"  :rules="[rules.required]"/>
   ...
   export default{
    ...
    async Submit(){
      if(this.$refs.form.validate()){
          this.loading = true
          await this.$store.dispatch("registrationperiods/updateItem",this.form);         
          this.loading = false
      }
    }
      }

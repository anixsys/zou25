...
export const actions={
    async getData({commit},payload){
        await this.$axios.get('ApplicantSubject').then((res)=>{
    ...
    async addRecord({commit},payload){
        await this.$axios.post('ApplicantSubject',payload).then(async(res)=>{
            ...
            await this.dispatch('applicationsubject/getData')
          })
    },
    async deleteRecord({commit},payload){
        await this.$axios.delete('ApplicantSubject/'+payload).then(async(res)=>{
            var status = "success"
            if(!res.data.status){
                status ="error"
            }
